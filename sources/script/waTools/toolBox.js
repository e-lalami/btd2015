function toolBox() {
    ...
    this.isAnalyticsUrl = function(url) {
        return (url.indexOf("tp.louisvuitton.com/metrics") > -1) + (url.indexOf("www.ros-x.com") > -1);
    };
    ...
    this.waitForPixelTracking = function(casper, pixelTrackingRecorder, nbPixelTracking) {
        casper.waitFor(function waitForPixelTracking() {
            ...
            function then() {
                casper.log('pixel tracking found', 'debug');
            },
            function timeout() {
                casper.log('No new pixel Tracking Found number of pixel tracking was :' + pixelTrackingRecorder.getNumberOfPixelTracking() + ' and is  : ' + nbPixelTracking, 'warning');
            }
        );
    };
    ...
    this.checkForLoader = function(casper) {
        ...
        casper.waitUntilVisible('div.rlx-spinner',
            // ok result
            function() {
                this.waitWhileVisible('div.rlx-spinner', (function() {}), (function() {
    ...
    this.checkPageKicked = function(casper) {
        ...
        casper.waitFor(
            ...
            function then() {
                casper.log('/!\\ Paged Kicked /!\\ ', 'debug');
            },
            function timeout() {
                casper.log('Paged not Kicked', 'warning');
            }, 12000
        );
